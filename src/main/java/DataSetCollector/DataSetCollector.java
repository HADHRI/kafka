...
public class DataSetCollector {
    ...
    private static final String OPEN_DATA_URL = "https://opendata.saemes.fr/api/records/1.0/search/?dataset=places-disponibles-parkings-saemes&rows=1000&sort=nom_parking&facet=date&facet=nom_parking&facet=type_de_parc&facet=horaires_d_acces_au_public_pour_les_usagers_non_abonnes&facet=countertype&facet=counterfreeplaces";

    private static final Logger logger = LoggerFactory.getLogger(DataSetCollector.class);
    private final KafkaPublisher publisher;

    public DataSetCollector(KafkaPublisher publisher) {
    ...
    void collect() {
        try {
            ...
            IntStream.range(0, 46).forEach(value -> {
